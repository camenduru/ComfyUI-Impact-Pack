...
SCHEDULERS = totoro.samplers.KSampler.SCHEDULERS + ['AYS SDXL', 'AYS SD1', 'AYS SVD']
...
# CREDIT: https://github.com/BlenderNeko/totoroUI_Noise/blob/afb14757216257b12268c91845eac248727a55e2/nodes.py#L68
    ...
    def doit_ipadapter(self, model):
        ...
        if 'IPAdapterAdvanced' not in nodes.NODE_CLASS_MAPPINGS:
            if 'IPAdapterApply' in nodes.NODE_CLASS_MAPPINGS:
                raise Exception(f"[ERROR] 'totoroUI IPAdapter Plus' is outdated.")

            utils.try_install_custom_node('https://github.com/cubiq/totoroUI_IPAdapter_plus',
                                          "To use 'IPAdapterApplySEGS' node, 'totoroUI IPAdapter Plus' extension is required.")
            raise Exception(f"[ERROR] To use IPAdapterApplySEGS, you need to install 'totoroUI IPAdapter Plus'")
    ...
    def apply(self, positive, negative, image, mask=None, use_acn=False):
        ...
        if use_acn:
            if "ACN_AdvancedControlNetApply" in nodes.NODE_CLASS_MAPPINGS:
                ...
                utils.try_install_custom_node('https://github.com/BlenderNeko/totoroUI_TiledKSampler',
                                              "To use 'ControlNetWrapper' for AnimateDiff, 'totoroUI-Advanced-ControlNet' extension is required.")
    ...
    def apply(self, positive, negative, image, mask=None, use_acn=False):
        ...
        if use_acn:
            if "ACN_AdvancedControlNetApply" in nodes.NODE_CLASS_MAPPINGS:
                ...
                utils.try_install_custom_node('https://github.com/BlenderNeko/totoroUI_TiledKSampler',
                                              "To use 'ControlNetAdvancedWrapper' for AnimateDiff, 'totoroUI-Advanced-ControlNet' extension is required.")
                raise Exception("'ACN_AdvancedControlNetApply' node isn't installed.")
        else:
...
# REQUIREMENTS: BlenderNeko/totoroUI_TiledKSampler
class TiledKSamplerWrapper:
    ...
    def sample(self, latent_image, hook=None):
        if "BNK_TiledKSampler" in nodes.NODE_CLASS_MAPPINGS:
            ...
            utils.try_install_custom_node('https://github.com/BlenderNeko/totoroUI_TiledKSampler',
                                          "To use 'TiledKSamplerProvider', 'Tiled sampling for totoroUI' extension is required.")
...
class PixelTiledKSampleUpscaler:
    ...
    def tiled_ksample(self, latent, images):
        if "BNK_TiledKSampler" in nodes.NODE_CLASS_MAPPINGS:
            ...
            utils.try_install_custom_node('https://github.com/BlenderNeko/totoroUI_TiledKSampler',
                                          "To use 'PixelTiledKSampleUpscalerProvider', 'Tiled sampling for totoroUI' extension is required.")
        ...
        if self.tile_cnet is not None:
            ...
            if image_batch > 1:
                ...
                if 'TilePreprocessor' not in nodes.NODE_CLASS_MAPPINGS:
                    raise RuntimeError("'TilePreprocessor' node (from totoroui_controlnet_aux) isn't installed.")
...
# REQUIREMENTS: biegert/totoroUI-CLIPSeg
class BBoxDetectorBasedOnCLIPSeg:
    ...
    def detect_combined(self, image, bbox_threshold, bbox_dilation):
        if "CLIPSeg" in nodes.NODE_CLASS_MAPPINGS:
            ...
            utils.try_install_custom_node('https://github.com/biegert/totoroUI-CLIPSeg/raw/main/custom_nodes/clipseg.py',
...
try:
    ...
    print(f"#########################################################################")
    print(f"[ERROR] totoroUI-Impact-Pack: Please update totoroUI to the latest version.")
    print(f"#########################################################################")
