...
class ImagePasteMasked:
    ...
    def composite(self, destination, source, x, y, resize_source, mask = None):
        ...
        output = totoro_extras.nodes_mask.composite(destination, source.movedim(-1, 1), x, y, mask, 1, resize_source).movedim(1, -1)
        return (output,)
        ...
        if len(mask) == 1:
            ...
            for mask2 in mask[1:]:
                ...
                if mask1.shape[1:] != mask2.shape[1:]:
                    mask2 = totoro.utils.common_upscale(mask2.movedim(-1, 1), mask1.shape[2], mask1.shape[1], "lanczos", "center").movedim(1, -1)
        ...
        if len(images) <= 1:
            ...
            for image2 in images[1:]:
                if image1.shape[1:] != image2.shape[1:]:
                    image2 = totoro.utils.common_upscale(image2.movedim(-1, 1), image1.shape[2], image1.shape[1], "lanczos", "center").movedim(1, -1)
