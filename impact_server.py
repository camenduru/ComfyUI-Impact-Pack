...
async def sam_prepare(request):
    ...
    with sam_lock:
        ...
        print(f"[INFO] totoroUI-Impact-Pack: Loading SAM model '{impact_pack.model_path}'")
        ...
        print(f"[INFO] totoroUI-Impact-Pack: SAM model loaded. ")
    return web.Response(status=200)
...
async def release_sam(request):
    ...
    print(f"[INFO] totoroUI-Impact-Pack: unloading SAM model")
...
def onprompt(json_data):
    try:
        ...
        print(f"[WARN] totoroUI-Impact-Pack: Error on prompt - several features will not work.\n{e}")
