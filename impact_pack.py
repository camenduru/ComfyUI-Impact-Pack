...
class CLIPSegDetectorProvider:
    ...
    def doit(self, text, blur, threshold, dilation_factor):
        if "CLIPSeg" in nodes.NODE_CLASS_MAPPINGS:
            ...
            print("[ERROR] CLIPSegToBboxDetector: CLIPSeg custom node isn't installed. You must install biegert/totoroUI-CLIPSeg extension to use this node.")
    ...
    def load_model(self, model_name, device_mode="auto"):
        if model_name == 'ESAM':
            if 'ESAM_ModelLoader_Zho' not in nodes.NODE_CLASS_MAPPINGS:
                try_install_custom_node('https://github.com/ZHO-ZHO-ZHO/totoroUI-YoloWorld-EfficientSAM',
                                        "To use 'ESAM' model, 'totoroUI-YoloWorld-EfficientSAM' extension is required.")
                raise Exception("'totoroUI-YoloWorld-EfficientSAM' node isn't installed.")
        ...
        device = totoro.model_management.get_torch_device() if device_mode == "Prefer GPU" else "CPU"
...
class DetailerForEach:
    ...
    def do_detail(image, segs, model, clip, vae, guide_size, guide_size_for_bbox, max_size, seed, steps, cfg, sampler_name, scheduler,
        ...
        if len(image) > 1:
            raise Exception('[Impact Pack] ERROR: DetailerForEach does not allow image batches.\nPlease refer to https://github.com/ltdrdata/totoroUI-extension-tutorials/blob/Main/totoroUI-Impact-Pack/tutorial/batching-detailer.md for more information.')
...
class NoiseInjectionDetailerHookProvider:
    ...
    def doit(self, schedule_for_cycle, source, seed, start_strength, end_strength):
        try:
            ...
            print("[ERROR] NoiseInjectionDetailerHookProvider: 'totoroUI Noise' custom node isn't installed. You must install 'BlenderNeko/totoroUI Noise' extension to use this node.")
            print(f"\t{e}")
            pass
...
class UnsamplerDetailerHookProvider:
    ...
    def doit(self, model, steps, start_end_at_step, end_end_at_step, cfg, sampler_name,
        ...
        try:
            ...
            print("[ERROR] UnsamplerDetailerHookProvider: 'totoroUI Noise' custom node isn't installed. You must install 'BlenderNeko/totoroUI Noise' extension to use this node.")
            print(f"\t{e}")
            pass
...
class UnsamplerHookProvider:
    ...
    def doit(self, model, steps, start_end_at_step, end_end_at_step, cfg, sampler_name,
        ...
        try:
            ...
            print("[ERROR] UnsamplerHookProvider: 'totoroUI Noise' custom node isn't installed. You must install 'BlenderNeko/totoroUI Noise' extension to use this node.")
            print(f"\t{e}")
            pass
...
class NoiseInjectionHookProvider:
    ...
    def doit(self, schedule_for_iteration, source, seed, start_strength, end_strength):
        try:
            ...
            print("[ERROR] NoiseInjectionHookProvider: 'totoroUI Noise' custom node isn't installed. You must install 'BlenderNeko/totoroUI Noise' extension to use this node.")
            print(f"\t{e}")
            pass
...
class PixelTiledKSampleUpscalerProvider:
    ...
    def doit(self, scale_method, model, vae, seed, steps, cfg, sampler_name, scheduler, positive, negative, denoise, tile_width, tile_height, tiling_strategy, upscale_model_opt=None, pk_hook_opt=None, tile_cnet_opt=None):
        if "BNK_TiledKSampler" in nodes.NODE_CLASS_MAPPINGS:
            ...
            print("[ERROR] PixelTiledKSampleUpscalerProvider: totoroUI_TiledKSampler custom node isn't installed. You must install BlenderNeko/totoroUI_TiledKSampler extension to use this node.")
...
class PixelTiledKSampleUpscalerProviderPipe:
    ...
    def doit(self, scale_method, seed, steps, cfg, sampler_name, scheduler, denoise, tile_width, tile_height, tiling_strategy, basic_pipe, upscale_model_opt=None, pk_hook_opt=None, tile_cnet_opt=None):
        if "BNK_TiledKSampler" in nodes.NODE_CLASS_MAPPINGS:
            ...
            print("[ERROR] PixelTiledKSampleUpscalerProviderPipe: totoroUI_TiledKSampler custom node isn't installed. You must install BlenderNeko/totoroUI_TiledKSampler extension to use this node.")
    ...
    def doit(self, image, link_id, save_to_workflow, image_data, trigger_always):
        if save_to_workflow:
            try:
                ...
                print(f"[WARN] totoroUI-Impact-Pack: ImageReceiver - invalid 'image_data'")
                mask = torch.zeros((64, 64), dtype=torch.float32, device="cpu")
...
class LatentSender(nodes.SaveLatent):
    ...
    def prepare_preview(latent_tensor, preview_method):
        from totoro.cli_args import LatentPreviewMethod
...
class ImpactSchedulerAdapter:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            "scheduler": (totoro.samplers.KSampler.SCHEDULERS, {"defaultInput": True,}),
